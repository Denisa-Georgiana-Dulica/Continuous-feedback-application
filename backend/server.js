...
app.get('/teacher-activities',async (req,res)=>{
        ...
        const teacher_activities=await activity.findAll({
                where:{teacherId:req.auth.userId},
                order:[['start_date','ASC']]
        });

        if(teacher_activities.length===0)
        {
            return res.status(200).json({message:'No activities for this teacher'});
        }

        res.status(200).json({message:"Teacher's activities:",teacher_activities});
    }
    catch(e)
    {
        console.log('Error fetching teacher activities:',e);
